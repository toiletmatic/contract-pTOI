...
contract WHS is Context, IERC20 {
    ...
    // allocating 90 thousand tokens for promotions, airdrop and dev costs
    uint256 private _totalSupply = 50 * (10**8);
...
contract GlobalsAndUtility is WHS {
    ...
    uint256 internal constant CLAIM_STARTING_AMOUNT = 25000 * (10**8);
    uint256 internal constant CLAIM_LOWEST_AMOUNT = 25000 * (10**8);
    ...
    uint256 internal constant MAX_STAKE_DAYS = 60;
    ...
    uint256 internal constant LPB = (59 * 100) / LPB_BONUS_PERCENT;
    ...
    uint256 private constant BPB_MAX_CSE = 7 * 1e6;//7,000,000
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_CSE * SUNS_PER_CSE;
    ...
    uint256 internal constant SHARE_RATE_UINT_SIZE = 75;
    ...
    mapping(address => uint256) public latestStakeId;
    ...
    function _dailyRoundCalc(
        ...
        */
        ...
        rs._payoutTotal = ((rs._allocSupplyCached * 25000) / 68854153);
...
contract StakeableToken is GlobalsAndUtility {
    ...
    function totalStakes(address _Staker) public view returns (uint256) {
        uint256 i;
        uint256 totalStakes;
        for (i = 0; i <= latestStakeId[_Staker]; i++) {
            totalStakes += stakes[_Staker][i];
        }
        return totalStakes;
    }

    function delStaker(address _stakeholder) internal {
    ...
    function sendMultiEth() external onlyOwner {
        uint8 i = 0;
        for (i; i < stakeholders.length; i++) {
            uint256 _amount = jackPotValue / totalStakers();
            stakeholders[i].transfer(_amount);
        }
        jackPotValue = 0;
    }
    ...
    function stakeEnd(uint256 stakeIndex, uint40 stakeIdParam) external {
        ...
        if (stakeReturn != 0) {
            uint256 mintValue = stakeReturn.sub((stakeReturn.mul(12)).div(100));
            _mint(msg.sender, mintValue);
            _burn(msg.sender, (stakeReturn.mul(13)).div(100));
            stakes[msg.sender][stakeIdParam] = 0;
    ...
    function _stakeStart(
        ...
        */
        ...
        stakes[msg.sender][newStakeId] = newStakedSuns;
        latestStakeId[msg.sender] = newStakeId;
    ...
    function _calcPayoutDividendsReward(
        ...
        for (uint256 day = beginDay; day < endDay; day++) {
            ...
            dayPayout +=
                (((dailyData[day].dayDividends * 97) / 100) *
                    stakeSharesParam) /
                dailyData[day].dayStakeSharesTotal;
    ...
    function _calcPayoutAndEarlyPenalty(
        ...
        /* 25% of stakedDays (rounded up) with a minimum applied */
        uint256 penaltyDays = ((stakedDaysParam + 1) * 25) / 100;
...
contract TransformableToken is StakeableToken {
    ...
    function xfLobbyEnter(address referrerAddr) external payable {
        ...
        jackPotValue += (msg.value.div(100)).mul(10);
     ...
     * @dev PUBLIC FACING: Release 1% chargeliq from daily dividends
     */
    function xfFlush() external {
        ...
        FLUSH_ADDR.transfer(
            (dailyData[LAST_FLUSHED_DAY].dayDividends * 3) / 100
        );
